...
def get_remaining_events(season: int, after_round: int) -> pd.DataFrame:
    ...
    contains_sprint = fmt.notna() & fmt.str.contains("sprint", case=False, na=False) if fmt is not None else pd.Series(False, index=remain.index)

    # 2) fallback via noms de sessions (Session1..Session5)
    session_cols = [c for c in remain.columns if c.startswith("Session") and not c.endswith("Date") and not c.endswith("DateUtc")]
    ...
    keep = ["RoundNumber", "Country", "Location", "EventName", "EventFormat", "is_sprint"] + session_cols
...
def compute_points_pool(remain: pd.DataFrame) -> Tuple[int, int, int]:
    ...
    total_points = sprint_events * POINTS_FOR_SPRINT_WEEKEND + conventional_events * POINTS_FOR_CONVENTIONAL
...
def main() -> None:
    ...
    fmt_uniques = remain["EventFormat"].dropna().unique().tolist() if "EventFormat" in remain else []
    ...
    print(f"[debug] remaining={len(remain)} (sprint={sprint_count}, conventional={conv_count}) "
    ...
    for _, d in standings.iterrows():
        ...
        if max_points_remaining > 0:
            ...
            pct_needed = 0.0 if current >= leader_points else round(points_needed / max_points_remaining * 100, 1)
