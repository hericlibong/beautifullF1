...
- **Always ask FastF1 for a session by round number, never by GP name** — `fastf1.get_session()` fuzzy-matches names, and "Spain" resolves to the *Spanish GP* (Madrid, round 14), not Barcelona (round 7). `race_chart_builder_fastf1.py` and `build_qualifying_data.py` both pass `r["round"]`. This assumes `calendar_2026.json` round numbers match FastF1's — they do, the file is generated by `fetch_calendar.py` from the FastF1 schedule (23 rounds in 2026, including round 16, the Bahrain GP held at Sepang).
