...
function render(drivers, gpColumns, scaleType = "linear", mode = "cumul") {
  const margin = { top: 20, right: 140, bottom: 130, left: 50 };
  const width  = 1180;
  ...
  function updateSliderUI(t) {
    if (!slider) return;
    if (document.activeElement !== slider) slider.value = Math.round(t * 1000);
    if (sliderLabel) {
      const nGp = gpColumns.length;
      const idx = Math.min(nGp - 1, Math.max(0, Math.floor(t * nGp - 0.0001)));
      sliderLabel.textContent = t <= 0 ? "Départ" : gpColumns[idx];
    }
  }
...
function buildQuickFilters(drivers) {
  ...
  buttons.on("click", function(event, d) {
    ...
    d3.selectAll(".qf-btn").classed("active", function() { return this.dataset.filterId === activeFilter; });
    applySelection(selected);
  });
