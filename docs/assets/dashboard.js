...
function aggregateHistoryWinners(history, teamColor) {
  ...
  for (const agg of byWinner.values()) {
    ...
    for (const [k, c] of agg.teamCounts) {
      if (c > bestCount) { bestCount = c; bestTeam = k; }
    }
    const sample = history.editions.find(e => e.winner === agg.winner && (e.teamId || e.team) === bestTeam);
...
function renderHistoryBars(history, teamColor) {
  const list = aggregateHistoryWinners(history, teamColor);
  if (!list.length) return "";
  const maxWins = list[0].wins;
  const rows = list.map(d => {
    const pct = (d.wins / maxWins) * 100;
    const initials = d.winner.split(/\s+/).map(w => w[0]).filter(Boolean).slice(0, 2).join("").toUpperCase();
...
function wireCircuitHistory(detail, history, teamColor) {
  ...
  const setActiveWinner = (name) => {
    plot.querySelectorAll(".dash-history-sq").forEach(s => {
      if (!name) { s.classList.remove("is-dim", "is-active"); return; }
      if (s.dataset.winner === name) {
        s.classList.add("is-active"); s.classList.remove("is-dim");
      } else {
        s.classList.add("is-dim"); s.classList.remove("is-active");
      }
    });
    root.querySelectorAll(".dash-history-bar-row").forEach(b => {
      if (!name) { b.classList.remove("is-dim", "is-active"); return; }
      if (b.dataset.winner === name) {
        b.classList.add("is-active"); b.classList.remove("is-dim");
      } else {
        b.classList.add("is-dim"); b.classList.remove("is-active");
      }
    });
  };
  ...
  plot.addEventListener("mouseover", (ev) => {
    ...
    if (sq) { showTip(+sq.dataset.i, sq); setActiveWinner(sq.dataset.winner); }
  });
  plot.addEventListener("mouseout", (ev) => {
    const sq = ev.target.closest(".dash-history-sq");
    if (!sq) return;
    const next = ev.relatedTarget && ev.relatedTarget.closest && ev.relatedTarget.closest(".dash-history-sq");
    if (next !== sq) { hideTip(); setActiveWinner(null); }
  });
  ...
  plot.addEventListener("focusout", (ev) => {
    const sq = ev.target.closest(".dash-history-sq");
    if (sq) { hideTip(); setActiveWinner(null); }
  });
  ...
  if (barsList) {
    ...
    barsList.addEventListener("mouseout", (ev) => {
      const row = ev.target.closest(".dash-history-bar-row");
      if (!row) return;
      const next = ev.relatedTarget && ev.relatedTarget.closest && ev.relatedTarget.closest(".dash-history-bar-row");
