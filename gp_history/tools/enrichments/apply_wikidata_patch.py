...
def apply_patch(gp_csv_in: Path, patch_csv: Path, gp_csv_out: Path) -> int:
    ...
    fill_mask = before_missing & merged["WinnerImageURL_patch"].notna() & (merged["WinnerImageURL_patch"] != "")
    merged.loc[fill_mask, "WinnerImageURL"] = merged.loc[fill_mask, "WinnerImageURL_patch"]

    merged = merged.drop(columns=[c for c in merged.columns if c.endswith("_patch")], errors="ignore")
...
if __name__ == "__main__":
    ...
    if not PATCH_CSV.exists():
        raise FileNotFoundError(f"Introuvable: {PATCH_CSV} (exécute wikidata_fetch.py ou copie ton query.csv)")
